...
export default function EditableDocumentTitle() {
  const [title, setTitle] = useState("Untitled Document")
  const [isEditingTitle, setIsEditingTitle] = useState(false)
  const [saveStatus, setSaveStatus] = useState("") // "saving" | "saved" | ""
  const titleTimeoutRef = useRef(null)

  // Simulated document ID - in your app, get this from useParams
  ...
  const updateDocumentTitle = async (newTitle) => {
    try {
      ...
      // In your actual app, get token from Clerk's useAuth hook
      ...
      const response = await fetch(`${backendUrl}/documents/${documentId}`, {
        ...
        headers: {
          ...
          // 'Authorization': `Bearer ${token}` // Uncomment in real app
        },
  ...
  const handleTitleBlur = () => {
    ...
    // Trigger immediate save on blur if there's a pending update
    if (titleTimeoutRef.current) {
      clearTimeout(titleTimeoutRef.current)
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="py-10 px-10 max-w-4xl mx-auto">
        <div className="bg-white p-8 rounded-lg shadow-sm min-h-96">
          <h2 className="mt-0 text-2xl font-semibold text-gray-800 mb-4">How it works</h2>
          <ol className="space-y-3 text-gray-700 leading-relaxed">
            <li className="pl-2"><strong>Click the title</strong> at the top to start editing</li>
            <li className="pl-2"><strong>Type your new title</strong> - it will auto-save after 1 second of inactivity</li>
            <li className="pl-2"><strong>Press Enter or click away</strong> to immediately save</li>
            <li className="pl-2">Watch the save status indicator (Saving... → ✔ Saved)</li>
          </ol>

          <div className="mt-8 p-4 bg-gray-50 rounded-lg border-l-4 border-blue-500">
            <h3 className="mt-0 text-base font-semibold text-gray-800 mb-3">
              📌 Integration Notes
            </h3>
            <ul className="space-y-2 text-gray-700 text-sm leading-relaxed">
              <li>Uses <code className="bg-gray-200 px-2 py-0.5 rounded text-xs font-mono">PATCH /documents/:id</code> endpoint</li>
              <li>Debounced with 1-second delay (configurable)</li>
              <li>Separate from Socket.io content updates</li>
              <li>Updates database metadata for dashboard display</li>
              <li>Add Clerk authentication token in production</li>
            </ul>
          </div>
        </div>
      </div>
